...
def is_forced_end_turn_state(agent: Any, raw_state: dict) -> bool:
    ...
    action_mask = agent.battle_agent.valid_action_mask(raw_state)
    valid_action_keys = [
        agent.battle_agent.get_action_key(action_id)
...
def fold_reward_details(
    ...
    folded["auto_end_turns"] = len(auto_steps)
    folded["auto_end_turn_reward"] = sum(
