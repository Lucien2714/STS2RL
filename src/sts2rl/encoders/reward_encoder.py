"""Reward-screen encoder: rewards list, card rewards, and treasure relics."""
...
class RewardEncoder(CandidateEncoder):
  """Encode rewards / card_reward / treasure screens and enumerate their actions."""
  ...
  def encode_state(self, raw_state: dict, action_mask=None) -> list[float]:
    ...
    features.append(self._scale(len(self.valid_action_candidates(raw_state)), self.MAX_ITEMS))
    return [float(value) for value in features]

  def encode_action(self, raw_state: dict, action: dict) -> list[float]:
    action_type = action.get("type")
    features = self._action_type_features(action_type)
    features.append(self._scale(action.get("index", action.get("card_index", 0)), self.MAX_ITEMS))
  ...
  def _rewards_candidates(self, raw_state: dict) -> list[dict]:
    ...
    for fallback_index, item in enumerate(rewards.get("items", [])):
      index = self._parse_int(item.get("index", fallback_index), fallback_index)
  ...
  def _treasure_candidates(self, raw_state: dict) -> list[dict]:
    ...
    for fallback_index, relic in enumerate(treasure.get("relics", [])):
      index = self._parse_int(relic.get("index", fallback_index), fallback_index)
      candidates.append(self._candidate(
