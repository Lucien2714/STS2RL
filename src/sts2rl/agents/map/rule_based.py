"""Rule-based map-routing policy using weighted shortest paths."""
...
class MapPolicy(MapAgent):
    """Plan and follow a low-cost route through the map DAG."""
    ...
    def choose_action(self, state: dict) -> dict:
        """Choose the next map node, re-planning when the stored route is stale."""
