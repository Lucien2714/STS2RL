...
| `get_player_detail()` | General | Get local player stats, full deck, potions, relics, and combat piles |
...
For `state_type: "card_select"` and `state_type: "hand_select"`, `get_game_state(format="json")` includes `min_select`, `max_select`, `selected_count`, and `remaining_to_min`. If the selection requires an exact count, it also includes `required_select_count`. Markdown output prints the same requirement as `Selection needed: ...`.

Use `deck_select_card` / `mp_deck_select_card` for `card_select` until the count is satisfied, then confirm with `deck_confirm_selection` / `mp_deck_confirm_selection`. Use `combat_select_card` / `mp_combat_select_card` for `hand_select` until the count is satisfied, then confirm with `combat_confirm_selection` / `mp_combat_confirm_selection`.
...
| `mp_get_player_detail()` | General | Get local player stats, full deck, potions, relics, and combat piles |
