...
def _states() -> tuple[TrainingState, LoggingState]:
    ...
    logging = LoggingState(
        tensorboard_global_step=25,
        last_logged_episode=2,
        last_logged_optimizer_update=4,
    )
    return training, logging
...
def test_checkpoint_round_trip_restores_agent_rng_and_latest(tmp_path: Path):
    ...
    training, logging = _states()
    torch.manual_seed(123)

    path = manager.save_episode(agent, plan, training, logging)
...
def test_checkpoint_rejects_vocabulary_and_model_config_mismatch(tmp_path: Path):
    ...
    agent.optimizer_updates = 4
    training, logging = _states()
    manager.save_episode(agent, plan, training, logging)
...
def test_checkpoint_rejects_counter_disagreement(tmp_path: Path):
    run_dir = tmp_path / "run"
    vocabulary = GameVocabulary.from_bundled_data()
    plan = _plan(run_dir)
    manager = CheckpointManager(run_dir, vocabulary)
    ...
    with pytest.raises(CheckpointError, match="environment_steps"):
        manager.save_episode(agent, plan, training, logging)
...
def test_failed_save_keeps_previous_latest_and_removes_temporary_file(
    ...
    agent.optimizer_updates = 4
    training, logging = _states()
    original = manager.save_episode(agent, plan, training, logging)
    ...
    with pytest.raises(CheckpointError, match="failed to save"):
        manager.save("episode_000003.pt", agent, plan, training, logging)
